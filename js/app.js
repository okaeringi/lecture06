...
var showAlarmMessage = function(){
		...
		if(alarm.message.length > 0){//0以上の場合
				message = alarm.message;
		}
		if(Notification.permission == "granted"){//通知機能があった場合
...
var update = function(){
		...
		if(isReadyToCountdown()){//カウントダウンが始まっていてかつ0以上である場合
				updateCounter();
				window.setTimeout(update, INTERVAL);//INTERVALで定められた時間が経過したときに、updateを呼び出す
		}else{
				showAlarmMessage();//カウントダウンが終わった場合
		}
};
//カウントし続ける関数
...
var startAlarm = function(){
		...
		if(isReadyToCountdown()){
				updateCounter();
				window.setTimeout(update, INTERVAL);//カウントダウンの準備ができた場合
		}
};
//以下のクラスから動作を選択する
